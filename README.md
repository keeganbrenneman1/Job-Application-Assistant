...
- **v-next:**
  ...
  - Stage-specific context (later refinement — likely unifying v2's per-stage field and v3's opportunity-level field, e.g. tagging opportunity-level context as stage-specific when needed; not fully resolved) to support the tool as a screen to have open during calls for note taking and seeding context for the next round
